/*
 * qdisc_uaf_repro_bridge.c
 *
 * Pure-userspace reproducer for the qdisc_pkt_len_segs_init() stale
 * transport_header bug via Linux bridge forwarding.
 *
 ...
 *
 * Topology:
 *   /dev/net/tun (syzq0) -> ip6gretap (gt1) -> bridge (br0) -> veth0
 *
 ...
 * keeps the stale transport header, so dev_queue_xmit ->
 * qdisc_pkt_len_segs_init() dereferences skb->data + negative_offset and
 * crashes.  Disabling GRO is not required because the bridge forwarding path
 * does not reset the transport header before the clone reaches the qdisc.
 */
