...
static int setup_topology(void)
{
	...
	/*
	 * Disable GRO on the VXLAN device so that the decapsulated inner frame
	 * is not processed by inet_gro_receive(), which would reset
	 * skb->transport_header to the inner TCP header.
	 */
	run_cmd("ethtool -K %s gro off 2>/dev/null || true", VXLAN_IFNAME);
...
/*
 * The inner frame is a valid Ethernet/IPv4/TCP packet.  We disable GRO on the
 * VXLAN device so that inet_gro_receive() does not reset
 * skb->transport_header after decapsulation.  The bridge then forwards the
 * inner frame at L2, leaving the stale outer transport header in place.
 */
...
int main(void)
{
	...
	printf("inner: Ethernet(broadcast)/IPv4/TCP (GRO disabled on vxlan0)\n");
