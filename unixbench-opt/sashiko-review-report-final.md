# Sashiko Patchset Review Report — Final Verified Version

> **Verified final version.** This report was produced by cross-checking the v2 report against `review-ana.md` and the final tree. One correction from v2 is reflected here: Patch 15's unreachable non-NUMA dcache-walker branches and the now-unused `i_mmap_walk_dcache_locked()` helper have been removed, so the dcache-lock contract concern noted in the original review is moot.
...
items to informational/maintenance notes.  Additionally, the unreachable
non-NUMA dcache-walker branches introduced by Patch 15 have been removed, so
the dcache-lock contract concern from the original review is no longer
applicable.
...
#### Analysis
Patch 15 originally introduced `i_mmap_walk_dcache_locked()` and converted the
ARM, Nios II, and PA-RISC cache-coherency walkers to visit all shard roots
while holding `flush_dcache_mmap_lock()`.  In the final tree the unreachable
non-NUMA architecture branches and the now-unused helper declaration and
implementation have been removed: `I_MMAP_SHARDS` depends on `CONFIG_NUMA`
(`mm/Kconfig:5-7`), and `arch/arm`, `arch/nios2`, and `arch/parisc` do not
define `CONFIG_NUMA`, so the new `#ifdef CONFIG_I_MMAP_SHARDS` branches were
dead code.  The architecture files were restored to their pre-Patch 15 blobs,
and the dcache lock/unlock calls that were added only to serialize those walkers
were also removed.

#### 🟢 Unreachable dcache-walker code: REMOVED
1. **Non-NUMA dcache walker branches and helper removed**
   - **Location**: `arch/arm/mm/fault-armv.c`, `arch/arm/mm/flush.c`,
     `arch/nios2/mm/cacheflush.c`, `arch/parisc/kernel/cache.c`,
     `include/linux/fs.h`, `mm/mmap.c`
   - **Description**: `i_mmap_walk_dcache_locked()` and the architecture
     branches that called it are gone from the final tree.  On NUMA-capable
     architectures the existing `i_mmap_read_walk()` / `i_mmap_lock_read_all()`
     paths already provide the required serialization; on non-NUMA architectures
     the code falls back to the original central-tree implementation.
   - **Impact**: The mixed dcache-spinlock / shard-rwsem locking contract that
     the original review flagged as fragile no longer exists.
   - **Fix**: None required; cleanup is complete.
...
- Several subtle invariants (`i_mmap_nr_vmas` accounting, same mapping for
  `vp->insert`) are not asserted by helpers.
...
2. Add `BUILD_BUG_ON(!is_power_of_2(I_MMAP_SHARDS_PER_DOMAIN))` to guard the
   `hash_ptr()` shift.
3. Add `VM_WARN_ON_ONCE` or lockdep assertions for the `i_mmap_nr_vmas` and
   `vp->insert` mapping invariants.
...
2. **Unasserted invariants in shard-tree mutation**
   ...
   - `i_mmap_nr_vmas` and `vp->insert` mapping/shard consistency are correct in
     the current tree but not enforced by helpers or assertions.
...
*Report generated by Sashiko Agent for OpenCode — final verified version based on review-ana.md.*
